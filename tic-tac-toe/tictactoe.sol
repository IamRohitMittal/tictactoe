...
contract TicTacToe
{
    // Conditions imposed by Contract 
    // Only 1 Game can be maintained at a time
    ...
    function roomFree() constant public returns(bool) {
        return room.finish;
    }
    ...
    function isWaiting() constant public returns(bool) {
        return room.waiting;
    }

    function joinGame() public payable waiting() otherPlayer() checkBid() {
    ...
    function reclaim() public canReclaim() open() {
        if (room.waiting) {
